...
public class ShadowFileInputToDelimitedOutput implements TalendJob {
	...
	public static class RowStruct implements routines.system.IPersistableRow<RowStruct> {
		final static byte[] commonByteArrayLock_COSTOSHOSPITALARIA_ShadowFileInputToDelimitedOutput = new byte[0];
		static byte[] commonByteArray_COSTOSHOSPITALARIA_ShadowFileInputToDelimitedOutput = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_COSTOSHOSPITALARIA_ShadowFileInputToDelimitedOutput.length) {
					if (length < 1024
							&& commonByteArray_COSTOSHOSPITALARIA_ShadowFileInputToDelimitedOutput.length == 0) {
						commonByteArray_COSTOSHOSPITALARIA_ShadowFileInputToDelimitedOutput = new byte[1024];
					} else {
						commonByteArray_COSTOSHOSPITALARIA_ShadowFileInputToDelimitedOutput = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_COSTOSHOSPITALARIA_ShadowFileInputToDelimitedOutput, 0, length);
				strReturn = new String(commonByteArray_COSTOSHOSPITALARIA_ShadowFileInputToDelimitedOutput, 0, length,
						utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_COSTOSHOSPITALARIA_ShadowFileInputToDelimitedOutput, 0, length);
				strReturn = new String(commonByteArray_COSTOSHOSPITALARIA_ShadowFileInputToDelimitedOutput, 0, length,
						utf8Charset);
			}
			return strReturn;
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_COSTOSHOSPITALARIA_ShadowFileInputToDelimitedOutput) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				fileName_tFileOutputDelimited = (new java.io.File(
						"D:/Documentos/2022/Documentos/2022-1 Universidad/5. Base de datos avanzada/ProyectoFinal/COSTOSHOSPITALARIA/temp/preview/costos_atencion_hospitalaria_bucaramanga.csv"))
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = ShadowFileInputToDelimitedOutput.class.getClassLoader().getResourceAsStream(
					"costoshospitalaria/shadowfileinputtodelimitedoutput_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
